...
void Interface::Initial(int argc, char **argv)
{
	...
	{
		fprintf(fp_log,"feadim times (+ (257+MFCCdim) noise) context must be equal to layersizes[0]\n");
		exit(0);
	}
...
//	}	
		...
		for(j =para->fea_dim; j< para->layersizes[numlayers-1];j++){ //做这样的改变后，不需要更改fea_norm文件和run.pl里的各个参数
			swap32((int *) (&targori[2+j +i*(para->layersizes[numlayers-1] +2)]));
			//targori[2+j +i*(para->layersizes[numlayers-1] +2)] -= mean[j-para->fea_dim];
			targori[2+j +i*(para->layersizes[numlayers-1] +2)] -= 0.0f;
			//targori[2+j +i*(para->layersizes[numlayers-1] +2)] *= dVar[j-para->fea_dim]; //para->fea_dim=257+MFCCdim
			targori[2+j +i*(para->layersizes[numlayers-1] +2)] *= 1.0f; //para->fea_dim=257+MFCCdim
			//printf("targori[2+j +i*(para->layersizes[numlayers-1] +2)]=%f,",targori[2+j +i*(para->layersizes[numlayers-1] +2)]);			
		}		
		//printf("\n");
	}
...
//	}
    for(i =0;i < frames_need_read; i++){
		 ...
		 for(j =para->fea_dim; j< para->layersizes[numlayers-1];j++){
			swap32((int *) (&targori[2+j +i*(para->layersizes[numlayers-1] +2)]));
			targori[2+j +i*(para->layersizes[numlayers-1] +2)] -= 0.0f;
			targori[2+j +i*(para->layersizes[numlayers-1] +2)] *= 1.0f;
			//printf("targori[2+j +i*(layersizes[numlayers-1] +2)]=%f,",targori[2+j +i*(layersizes[numlayers-1] +2)]);			
		 }
