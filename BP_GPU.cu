...
float BP_GPU::CrossValid(int n_frames, const float* in, const float *targ)/////////////////////////////////////by yongxu
{
	...
	//float squared_err_speech=0.0f;/////////////////////////////////////////////by yongxu,两个输出
	//float squared_err_noise=0.0f;/////////////////////////////////////////////by yongxu，两个输出
...
//   printf ("\n");
	...
	for (i=0; i< n_frames; i+= bunchsize)//n_frames是该CV集的总帧数；bunchsize指的是一个bunch里有多少帧；然后每个bunch分别计算
	{
		...
		{
		  for(d=350;d<out_dims;d++)///////////////////////////////////////by yongxu, 我们的输出特征始终是out_dims维
		  {
...
//		  //计算语音部分的error
//					  for(d=0;d<out_dims/2;d++)///////////////////////////////////////by yongxu, 我们的输出特征始终是out_dims维
//		  {
//		    squared_err_speech = squared_err_speech + (out[j*out_dims+d]-targ[j*out_dims+d])*(out[j*out_dims+d]-targ[j*out_dims+d]);/////////////by yongxu, 特别注意：squared error是与correct_samples相反的概念
//			}
//			
//					  //计算噪声部分的error
//					  for(d=out_dims/2;d<out_dims;d++)///////////////////////////////////////by yongxu, 我们的输出特征始终是out_dims维
//		  {
//		    squared_err_noise = squared_err_noise + (out[j*out_dims+d]-targ[j*out_dims+d])*(out[j*out_dims+d]-targ[j*out_dims+d]);/////////////by yongxu, 特别注意：squared error是与correct_samples相反的概念
//			}
...
//			}
		  ...
		  //计算语音部分的error
					  for(d=0;d<257;d++)///////////////////////////////////////by yongxu, 我们的输出特征始终是out_dims维
		  {
		    squared_err_speech = squared_err_speech + (out[j*out_dims+d]-targ[j*out_dims+d])*(out[j*out_dims+d]-targ[j*out_dims+d]);/////////////by yongxu, 特别注意：squared error是与correct_samples相反的概念
			}
