...
__global__ void kernSubClean( int rows , int cols, const float *in_vec1, const float *in_clean, float *res_vec)
{
	 ...
	 if(i < rows)
	 {
	 	 ...
	 	 float energy_mfcc=0.0f;
	 	 	 	...
	 	 	 	for(j =0; j< 257;j++)
		 	{
		 energy_s= energy_s + (in_clean[cols *i +j])*(in_clean[cols *i +j]);
		 }
		 energy_s=energy_s/257.0f;
		 
		 	 	 	 	for(j =257; j< (cols-257);j++)
		 	{
		 energy_mfcc= energy_mfcc + (in_clean[cols *i +j])*(in_clean[cols *i +j]);
		 }
		 energy_mfcc=energy_mfcc/(cols-257-257);
		 
//		 	  for(j =350; j< cols;j++)
//		 	{
//		 energy_n= energy_n + (in_clean[cols *i +j])*(in_clean[cols *i +j]);
//		 }
//		 energy_n=energy_n/257.0f;
		 ...
		 res_vec[cols *i + j] = (2.0f/rows)*0.5f*(in_vec1[cols *i +j]-in_clean[cols *i +j]);
		 ...
		 //int ind = in_index[i];
		 //res_vec[cols *i + ind] = in_vec1[cols *i +ind] - 1.0f;
		 res_vec[cols *i + j] = (2.0f/rows)*0.3f*(in_vec1[cols *i +j]-in_clean[cols *i +j]);
		 res_vec[cols *i + j] = (res_vec[cols *i + j]/energy_mfcc);
		 //res_vec[cols *i + j] = 2.0f*(in_vec1[cols *i +j]-in_clean[cols *i +j]);
		 //printf("in kernSubClean, res_vec=%f ",res_vec[cols *i + j]);
		 }
		 
		 	 	 for(j =(cols-257); j< cols;j++)
		 	{ 	//res_vec[cols *i +j] = in_vec1[cols *i +j];
		 //int ind = in_index[i];
		 //res_vec[cols *i + ind] = in_vec1[cols *i +ind] - 1.0f;
		 res_vec[cols *i + j] = (2.0f/rows)*0.2f*(in_vec1[cols *i +j]-in_clean[cols *i +j]);
